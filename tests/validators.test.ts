...
import { validators, coerceQueryNumber, coerceQueryBoolean, coerceQueryDate, getOrCompileSchema, is, assert, assertGuard, validate } from '../src/runtime/validators.js';
...
describe( 'Validators', () => 
{
    ...
    describe( 'Structural', () => 
    {
        ...
        it( 'should validate contains and propertyNames', () =>
        {
            const isString: typeof validators.string = ( v, path, c ) => validators.string( v, path, c );

            expect( validators.contains([ 'a', 1, 'b' ], 'path', ctx, isString, 2 )).toEqual([ 'a', 1, 'b' ]);
            expect( ctx.success ).toBe( true );

            ctx.success = true;
            ctx.errors = [];
            validators.contains([ 1, 2 ], 'path', ctx, isString, 1 );
            expect( ctx.success ).toBe( false );
            expect( ctx.errors[0]?.error ).toMatch( /Contains<min:1>/ );

            ctx.success = true;
            ctx.errors = [];
            validators.contains([ 'a', 'b', 'c' ], 'path', ctx, isString, 1, 2 );
            expect( ctx.success ).toBe( false );
            expect( ctx.errors[0]?.error ).toMatch( /Contains<max:2>/ );

            ctx.success = true;
            ctx.errors = [];
            const keyOk: typeof validators.string = ( v, path, c ) =>
            {
                validators.string( v, path, c );

                if( typeof v === 'string' && !/^[a-z]+$/.test( v ))
                {
                    c.success = false;
                    c.errors.push({ path, value : v, error : 'Pattern' });
                }

                return v;
            };
            expect( validators.propertyNames({ aa : 1 }, 'path', ctx, keyOk )).toEqual({ aa : 1 });
            expect( ctx.success ).toBe( true );

            ctx.success = true;
            ctx.errors = [];
            validators.propertyNames({ A : 1 }, 'path', ctx, keyOk );
            expect( ctx.success ).toBe( false );
        });
    ...
    describe( 'Dynamic Runtime Schema Validation', () => 
    {
        ...
        it( 'should treat an empty schema as accepting any value', () =>
        {
            ...
            expect(() => getOrCompileSchema({ type : 'unknown' } as never )).toThrow(

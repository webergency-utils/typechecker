...
import { type ValidationContext, getOrCompileSchema } from '../src/runtime/validators.js';
...
describe( 'compileSchema', () => 
{
    ...
    describe( 'combinators', () => 
    {
        ...
        it( 'should enforce nested additionalProperties under allOf with outer strict mode', () =>
        {
            // Arrange
            const fn = getOrCompileSchema({
                allOf : [
            ...
            // Act
            fn({ nested : { a : 'x', extra : 1 } }, '', ctx );

            // Assert
            expect( ctx.success ).toBe( false );
            expect( ctx.errors.some( e => e.error.includes( 'PropertyNotAllowed' ))).toBe( true );
        });
    });
    ...
    describe( 'refs and const', () => 
    {
        ...
        it( 'should walk full JSON Pointer $ref paths', () =>
        {
            // Arrange
            const schema =
            {
            ...
            expect( ctx.success ).toBe( false );
            expect( ctx.errors[0].error ).toContain( 'string' );
        });

        it( 'should apply typeless keywords only when the instance matches', () =>
        {
            const minLen = getOrCompileSchema({ minLength : 5 });

            expect( minLen( 'abcde', '', ctx )).toBe( 'abcde' );
            expect( ctx.success ).toBe( true );
            ...
            ctx = { success : true, errors : [], mode : 'strict' };
            expect( minLen( 1, '', ctx )).toBe( 1 );
            expect( ctx.success ).toBe( true );

            const props = getOrCompileSchema({
                properties : { a : { type : 'string' } },
                required   : ['a']
            });
            ...
            ctx = { success : true, errors : [], mode : 'strict' };
            expect( props( 'skip', '', ctx )).toBe( 'skip' );
            expect( ctx.success ).toBe( true );
        });
    });
    ...
    describe( 'array and string schema options', () => 
    {
        ...
        {
            // Act / Assert
            expect(() => getOrCompileSchema( null as never )).toThrow( /Invalid JSON Schema/ );
            expect(() => getOrCompileSchema( 'string' as never )).toThrow( /Invalid JSON Schema/ );
            expect(() => getOrCompileSchema([] as never )).toThrow( /Invalid JSON Schema/ );
        });

        it( 'should enforce const together with type (transformer literal shape)', () =>
        {
            const fn = getOrCompileSchema({ type : 'string', const : 'folder' });

            expect( fn( 'folder', '', ctx )).toBe( 'folder' );
            expect( ctx.success ).toBe( true );
        ...
        it( 'should deep-equal object const values', () =>
        {
            const fn = getOrCompileSchema({ const : { a : 1 } });

            expect( fn({ a : 1 }, '', ctx )).toEqual({ a : 1 });
            expect( ctx.success ).toBe( true );
        ...
        it( 'should require keys listed in required even when absent from properties', () =>
        {
            const fn = getOrCompileSchema({
                type                 : 'object',
                properties           : {},
            ...
            expect( ctx.success ).toBe( false );
            expect( ctx.errors.some( e => e.path === 'a' || e.error.includes( 'a' ))).toBe( true );
        });

        it( 'should apply minItems to tuple items arrays', () =>
        {
            const fn = getOrCompileSchema({
                type     : 'array',
                items    : [{ type : 'string' }, { type : 'number' }],
            ...
            expect( ctx.success ).toBe( false );
            expect( ctx.errors.some( e => e.error.includes( 'MinItems' ) || e.error.includes( 'Tuple' ))).toBe( true );
        });

...
import { getCachedPattern, createSafeRegex, testRegex, isRegexSafe } from '../src/runtime/regex.js';
import { childPath, indexPath } from '../src/runtime/path.js';
import { generateParseCode } from '../src/engine/parse-generator.js';
import { compileAndTransform, emitAndImport } from './helpers/compile.js';
import { parse, serializer, stringify } from '../src/index.js';
import { buildParser } from '../src/transformer.js';
...
describe( 'Parse', () =>
{
    ...
    describe( 'Coercers', () =>
    {
        ...
        it( 'applyParseConstraints fills defaults, transforms, and checks', () =>
        {
            ...
            expect(() => applyParseConstraints({ a : 1 }, 'n', [{ type : 'minProperties', value : 2 }])).toThrow( /MinProperties/ );
            expect(() => applyParseConstraints({ a : 1, b : 2, c : 3 }, 'n', [{ type : 'maxProperties', value : 2 }])).toThrow( /MaxProperties/ );
            expect( applyParseConstraints({ a : 1, b : 2 }, 'n', [
                { type : 'minProperties', value : 2 },
                { type : 'maxProperties', value : 3 }
            ])).toEqual({ a : 1, b : 2 });
        ...
        it( 'applyContains / applyPropertyNames enforce nested parse checks', () =>
        {
            expect( applyContains(
                [ 'ab', 1, 'cd' ],
                'n',
                ( item, p ) =>
                {
                    ...
                    {
                        throw new ParseError( p, 'Type<string>' );
                    }
            ...
            expect(() => applyContains(
                ...
                ( item, p ) =>
                {
                    if( typeof item !== 'string' ){ throw new ParseError( p, 'Type<string>' ) }

                    return item;
                },
                1
            )).toThrow( /Contains<min:1>/ );
            ...
            expect(() => applyContains(
                ...
                1,
                2
            )).toThrow( /Contains<max:2>/ );

            expect( applyPropertyNames(
                { aa : 1 },
                'n',
                ( key, p ) =>
                {
                    ...
                    {
                        throw new ParseError( p, 'Pattern' );
                    }

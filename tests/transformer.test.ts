...
describe( 'Transformer Call Expression Replacements', () => 
{
    ...
    {
        const code = `
            import { validate as check, validateSchema as checkSchema } from '../src/index.js';
    ...
    {
        const code = `
            import * as typechecker from '../src/index.js';
    ...
    {
        const code = `
            import { assertGuard } from '../src/index.js';
            const x: unknown = 123;
    ...
    {
        const code = `
            import { is, assert } from '../src/index.js';
    ...
    it( 'should transform MinProperties / MaxProperties on object types', () =>
    {
        const code = `
            import { validate, jsonSchema, constraint } from './src/index.js';
            type Bag = Record<string, number> & constraint.MinProperties<2> & constraint.MaxProperties<5>;
            const res = validate<Bag>({ a: 1, b: 2 });
            const schema = jsonSchema<Bag>();
        `;
        const compiled = compileAndTransform( code );

        expect( compiled ).toMatch( /validators\.(minProperties|maxProperties)/ );
        expect( compiled ).toContain( '"minProperties": 2' );
        expect( compiled ).toContain( '"maxProperties": 5' );
    });

    it( 'should transform Contains / PropertyNames nested schema tags', () =>
    {
        const code = `
            import { validate, jsonSchema, constraint } from './src/index.js';
            ...
            const a = validate<Tags>(['ab', 'cd', 1]);
            const b = validate<Bag>({ aa: 1 });
            const s1 = jsonSchema<Tags>();
            const s2 = jsonSchema<Bag>();
        `;
        const compiled = compileAndTransform( code );

        expect( compiled ).toContain( 'validators.contains' );
        expect( compiled ).toContain( 'validators.propertyNames' );
        expect( compiled ).toContain( '"contains"' );
        expect( compiled ).toContain( '"minContains": 2' );
        expect( compiled ).toContain( '"maxContains": 3' );
        expect( compiled ).toContain( '"propertyNames"' );
        expect( compiled ).toContain( '"pattern": "^[a-z]+$"' );
    });
    ...
    {
        const code = `
            import { validate, is } from '../src/index.js';

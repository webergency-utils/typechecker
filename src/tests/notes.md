...
- When any union arm carries `tag.Default`, a sibling bare `undefined` arm is dropped before emit. `optional()` would short-circuit before the default runs, and a leading `undefined` arm in a plain union would win the same way — so `port?: number & tag.Default<8080>` compiles to the Defaulted validator alone (still marked `hasDefault` on the props tuple).
- Object unions sharing a distinct string/number literal property compile to `validators.taggedUnion`, which dispatches on the tag and reports the selected arm's errors directly. A non-object, an unknown tag, or a tag still needing `from` coercion falls back to `union` (so numeric tags from a querystring still resolve).
- An absent optional property is skipped entirely by `validators.props` — no phantom `undefined` key on the result, no union attempt. The props tuple carries a fourth `hasDefault` element (emitted only when true) for the one case that must still run: `port?: number & tag.Default<8080>`. If that Defaulted run fails a constraint, the error is kept (no rollback).
- Compile harnesses must pass `types: ['node']`. `ResolveDefaults` names `Buffer` in its exclusion list; without node types the alias collapses to `any` and `assert<ResolveDefaults<…>>` emits `validators.any`.
- Property types on mapped results (`ResolveDefaults`, `ConvertPropertyCasing`) are read via `getTypeOfSymbol` — those symbols often have no declaration, and the old `getTypeOfSymbolAtLocation` fallback returned `any`, dropping Defaults.
- `validators.objectShell( v, ctx, closed )` returns `{}` for a closed shape unless the mode is `relaxed`, since a closed shape writes back every key it keeps. Symbol-keyed properties are therefore not carried over on closed shapes. `createIntersectionCheck` leaves `closed` unset and keeps the copy.
- Tag detection matches the `TAG_KEYS` registry in `src/engine/tagKeys.ts`, not a `__` prefix, so a real `__typename` property is validated as data.
...
- `parse<any>` / `parse<unknown>` keep the json/query decode preamble but use a passthrough body (parity with `validators.any`) — they must not fall through to empty-object strip.
- `stringify` / serializers treat nested (and root) `any` / `unknown` as opaque: JSON via `serializeAny` (`undefined` → `null`), query via `appendQueryAny` (deep bracket encode). They must not fall through to empty-object emit.
- Assert ↔ parse behavioural parity (success data + canonical failure path/code for JSON and `from:'query'`, including Custom transform/constraint and unions) lives in `assert-parse-parity.test.ts`. `parse` options (`mode` / `from`) are compile-time only — call sites must use object literals. Validation error paths have **no** leading `.` (`'name'`, `'items[0]'`); array indices stay bracketed. Serialize/stringify error codes match assert (`Type<*>`, `Tuple<n>`, `PropertyNotAllowed<k>`; missing required → field `Type<*>`). Parse codegen uses helpers (`expectString` / `expectNumber` / `expectBoolean` / `expectObject` / `expectArray` / `parseUnion`). Union alias labels may still differ (`Type<Alias>` vs `Type<Union>`) and are canonicalized in that suite; `Pattern<'x'>` quotes are normalized the same way. Constraint phantoms for `jsonSchema` / static asserts share `collectConstraintsFromProps` with validators. Published runtime subpaths: `@webergency-utils/typechecker/runtime` (barrel), `/runtime/validators`, `/runtime/parse`, `/runtime/serialize`, `/runtime/casing`, `/runtime/path`, `/runtime/regex` (additive; barrel still re-exports).
- Shared type walk for validators + serialize/parse lives in `engine/type-helpers.ts` (`peelTaggedIntersection`, `tryMergeObjectTypes`, `tryTaggedUnionTypes`, `getPropertyType`, …). `resolver.ts` emits AST from that analysis; do not reintroduce parallel peel/merge/tag scrapers there.
...
- `npm test` / `npm run coverage` run `npm run build` first. Unit tests still import `src/` (so coverage instruments source); `emitAndImport` loads the built `dist/` runtime — that is the seam that must match what we publish.
- Compile harnesses live in `src/tests/helpers/compile.ts`. `compileAndTransform` prints the transformed AST; `emitWithTransformer` runs the real emit pipeline and is the only way to observe TypeScript's import elision, which happens after our transformer. `emitAndImport` rewrites runtime imports to `dist/` (absolute `file:` URLs). Give each suite its own temp name — test files run in parallel from the package root.
- `ResolveDefaults` type behaviour is asserted with vitest `expectTypeOf` in `tags.test.ts` (esbuild erases annotations, so plain `const x: Resolved = …` is not enough). Runtime Default filling — including asserting a `ResolveDefaults<…>` type directly — lives in `defaults-regression.test.ts`.
- Casing × Defaults × original optionality (AppConfig / PollerConfig patterns) live in `casing-defaults.test.ts`: `ConvertPropertyCasing` alone keeps original `?`; `ResolveDefaults<ConvertPropertyCasing<…>>` makes only Defaulted keys required; runtime covers assert-original-then-`convertPropertyCasing` and assert-resolved-cased-shape.

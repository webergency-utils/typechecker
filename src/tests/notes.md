...
- An absent optional property is skipped entirely by `validators.props` — no phantom `undefined` key on the result, no union attempt. The props tuple carries a fourth `hasDefault` element (emitted only when true) for the one case that must still run: `port?: number & tag.Default<8080>`.
...
- Compile harnesses live in `src/tests/helpers/compile.ts`. `compileAndTransform` prints the transformed AST; `emitWithTransformer` runs the real emit pipeline and is the only way to observe TypeScript's import elision, which happens after our transformer. Give each suite its own temp name — test files run in parallel from the package root.

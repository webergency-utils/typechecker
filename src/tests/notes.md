...
- Compile harnesses live in `src/tests/helpers/compile.ts`. `compileAndTransform` prints the transformed AST; `emitWithTransformer` runs the real emit pipeline and is the only way to observe TypeScript's import elision, which happens after our transformer. `emitAndImport` builds on that for end-to-end runtime checks against `dist/` (run `npm run build` first). Give each suite its own temp name — test files run in parallel from the package root.
